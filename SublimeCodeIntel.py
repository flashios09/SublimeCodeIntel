    ...
    def set_call_tip_info(self, buf, calltip, explicit, trg):
        def _set_call_tip_info():
                        ...
                        if p:
                            ...
                            if sep:
                                ...
                                if arg[1]:
                                    ...
                                    tvar = "<var class='arg_value'>%s</var>" % (tvar)
                            # if i == arguments:
                            #     tvar = "<b>%s</b>" % tvar
                            tip0.append(tvar)
                            if i >= arguments:
            ...
            if hasattr(view, 'show_popup'):
                ...
                view.show_popup('<style>%s</style>%s<br><br><a href="insert">Insert</a>' % (css, "<br>".join(measured_tips)), location=-1, max_width=1000, max_height=1000, on_navigate=insert_snippet)
                # view.show_popup("<br>".join(measured_tips), location=-1, max_width=1000, max_height=1000)
